...
class Cable(Entity):
    ...
    def _create_cable_solid(self,
        ...
        initial_ternal_shell = self._create_hole_shell(
            center_pnt=tcol_pnt.Value(1),
            normal_dir=gp_Dir(tcol_vec.Value(1)),
            diameter=diameter,
            thickness=thickness)
        final_ternal_shell = self._create_hole_shell(
            center_pnt=tcol_pnt.Value(tcol_pnt.Length()),
            normal_dir=gp_Dir(tcol_vec.Value(tcol_pnt.Length())),
            diameter=diameter,
            thickness=thickness)  
        
        inner_cable_shell = self._create_curve_shell(
            central_wire=central_curve_wire,
            center_pnt=tcol_pnt.Value(1),
            normal_dir=gp_Dir(tcol_vec.Value(1)),
            diameter=diameter)
        if inner_cable_shell == None:
            print("CableModeling: Inner Cable Shell is None.")
            return None
        ...
        sewing_builder.Add(inner_cable_shell)
        sewing_builder.Add(outer_cable_shell)   
        sewing_builder.Add(initial_ternal_shell)    
        sewing_builder.Add(final_ternal_shell)
